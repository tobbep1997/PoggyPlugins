...
@PluginDescriptor(
        name = "<html><font color=\"#FF9DF9\">[PP]</font> Bob The Skiller</html>",
        description = "Will interact with an object and drop or bank all items when inventory is full",
        tags = {"ethan", "piggy", "skilling"}
)
public class BobTheSkillerPlugin extends Plugin {
    @Inject
    private Client client;
    @Inject
    BobTheSkillerConfig config;
    @Inject
    ...
    @Provides
    private BobTheSkillerConfig getConfig(ConfigManager configManager) {
        return configManager.getConfig(BobTheSkillerConfig.class);
    }
